classdef AMCL2
    ...
    methods 
        function obj = AMCL2(map, dt, n_initial_particles, init_pose)       
            obj.map = map;
        ...
        function obj = update(obj, range_measurements, sensors)            
            maxDecrease = 10;
            logOddsDecrease = maxDecrease / length(range_measurements); % if no cell is found
            ...
            for (j = 1:size(obj.particles, 2))
                ...
                weightOdd = log(obj.weights(j));
                
                for (i = 1:length(range_measurements))                                        
                    [cell_value, cell_found] = sensors{i}.inverse_measurement_hit(range_measurements(i), obj.particles(:,j), obj.map);
                    
                    if (cell_found)
                        if (cell_value > 0) % cell is occupied
                            cellOdd = log(cell_value + 1);
                            logOddsIncrease = cellOdd / length(range_measurements);
                            weightOdd = weightOdd + logOddsIncrease;
                        else
                            weightOdd = weightOdd - logOddsDecrease;
                        end
                    else
                        weightOdd = weightOdd - logOddsDecrease;
                    end
                ...
                newWeight = exp(weightOdd);                
                obj.weights(j) = newWeight;
            end             
