classdef UKF
    ...
    methods
        ...
        function obj = predict(obj, varargin) 
            ...
            [obj.x, obj.P] = obj.UnscentedTransform(obj.f, obj.x, u, obj.P, obj.Q);
        end
    ...
    methods (Static, Access = private)              
        function [Mu_y, Cov_yy, Cov_xy] = UnscentedTransform(f, Mu_x, u, P, Q, varargin)
            % y = f(x,u,q)
            %   where x ~ N(x, P)
            %   and   q ~ N(0, Q)
            ...
            sigmaPoints(:,1) = Mu_prime;                        
            for (i = 1:n)
                sigmaPoints(:,1+i) = Mu_prime + sqrt(n / (1-W0)) * sqrtCov(:,i);
                sigmaPoints(:,1+i+n) = Mu_prime - sqrt(n / (1-W0)) * sqrtCov(:,i);
            end
