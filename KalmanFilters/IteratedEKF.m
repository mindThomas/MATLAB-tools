classdef IteratedEKF
    % See "6.2.3 The Iterative extended Kalman filter" from Course ChM015x
    % See also https://github.com/mvdhoek/IEKF/blob/master/ext_kalman.m
    ...
    methods
        function obj = IteratedEKF(varargin)                        
        ...
        function obj = update(obj, z)
            % Iterated Updating scheme
            %
            % 1. Perform the Update on the Prior using the prior as linearization point for the measurement model
            % 2. Use the resulting posterior estimate as a new linearization point for the measurement model
            % 3. Perform the Update again on the original Prior but by using the new linearization point
            % 4. Repeat until convergence
            %
            % Note that each update start with the predicted density to
            % avoid using the measurement multiple times
            x_prev = obj.x; % take the prior state estimate and covariance
            P_prev = obj.P;
            % Note that obj.x continues to hold the prior until after all iterations                        
            
            e = 1e-10; % convergence tolerance
            max_iter = 100;
            x_prev_old = x_prev;
            iter = 0;   
            while (iter == 0) || ((norm(x_prev - x_prev_old, Inf) / norm(x_prev_old, Inf) >= e) && (iter <= max_iter))
                x_prev_old = x_prev;
                [x_prev, P_prev] = obj.update_iteration(x_prev, z);
    ...
    methods (Access = private)
        ...
        function [x, P] = update_iteration(obj, x_linearization, z)
            % Measurement model is on the form:
            %   z = h(x, r)          [ defined with h = @(x,r) ... ]
            %   where r ~ N(0, R)
            % Which is linearized into
            %   z = h(x_linearization, 0) + Hx*(x_prior-x_linearization) + Hr*r
            % where:
            %   Hx = dh(x,r)/dx|x=x0,r=0
            %   Hr = dh(x,r)/dr|x=x0,r=0
            % Note how the linearization point is changed iteratively
            % according to the previous updated state, x_prev
            r = zeros(size(obj.R,1),1); % no noise
            if (obj.JacobiansAvailable)
                dhdx = obj.Hx(x_linearization, r);
                dhdr = obj.Hr(x_linearization, r);
            else
                [dhdx, dhdr] = numjacobian2(obj.h, x_linearization, r);  
            end            
            H = dhdx;
            ...
            z_hat = obj.h(x_linearization, r) + H*(obj.x - x_linearization); % Note how this is slightly different from the usual update step
            % Compute innovation
            innov = z - z_hat;
            ...
            %P = obj.P - obj.K * obj.S * obj.K';
            P = obj.P - obj.K * H * obj.P;
        end
    end
end