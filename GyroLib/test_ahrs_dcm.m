function [Euler_, bw_] = test_ahrs_dcm(dWb, Fb, Mb, q_ref, q_init, fn, mn, dt)
%  [Euler_, bw_] = test_ahrs_dcm(dWb, Fb, Mb, q_ref, q_init, dt)
%  Runs the sumulation for the DCM-based AHRS algorithm
%
%   Input arguments:
...
for i=1:Nsim
    ...
    %Angles errors
    Cbn_ref = quat_dcm(q_ref(i,:));
    Cerr = Cbn_ref*Cbn';
    [err_(i,1), err_(i,2), err_(i,3)] = dcm_angle(Cerr);
